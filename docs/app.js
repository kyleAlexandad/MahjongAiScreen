...
// Optional build-time config (docs/config.js for the static GitHub Pages
// build). Absent in the FastAPI dev server -> behaviour is unchanged.
const MJ_CONFIG = (typeof window !== "undefined" && window.MJ_CONFIG) || {};
const TILE_IMG_BASE = MJ_CONFIG.tileBase || "/tiles";
const API_BASE = MJ_CONFIG.apiBase || "/api";
// Static mode: no backend, the pure-Python engine runs in-browser via
// Pyodide (see docs/pyodide-boot.js, which sets window.MJEngine*).
const STATIC_MODE = !!MJ_CONFIG.static;

// Single seam between the UI and the engine. Server mode hits the HTTP
// API exactly as before; static mode calls the in-browser bridge. Both
// return a uniform { ok, status, data, detail } shape.
const Engine = {
  async _call(method, payloadObj) {
    if (STATIC_MODE) {
      await window.MJEngineReady;
      const arg = payloadObj === undefined ? undefined : JSON.stringify(payloadObj);
      const out = JSON.parse(window.MJEngine[method](arg));
      return out.ok
        ? { ok: true, status: 200, data: out.data }
        : { ok: false, status: out.status || 400, detail: out.detail };
    }
    const map = {
      tiles_json: ["GET", "/tiles"],
      analyze_json: ["POST", "/analyze"],
      analyze_game_json: ["POST", "/analyze-game"],
      analyze_call_json: ["POST", "/analyze-call"],
    };
    const [verb, path] = map[method];
  ...
  tiles() { return this._call("tiles_json"); },
  analyze(body) { return this._call("analyze_json", body); },
  analyzeGame(body) { return this._call("analyze_game_json", body); },
  analyzeCall(body) { return this._call("analyze_call_json", body); },
};
...
async function loadTileMetadata() {
  const r = await Engine.tiles();
  if (!r.ok) throw new Error(`tile metadata failed (${r.status})`);
  tileMetadata = new Array(34);
  for (const t of r.data) tileMetadata[t.tile_id] = t;
}
...
async function quickAnalyze() {
  ...
  try {
    const r = await Engine.analyze({ hand: idsFromCounts(state.quick.hand) });
    if (!r.ok) {
...
async function fetchCallAnalysis(tileId, discarderSeat) {
  try {
    const r = await Engine.analyzeCall({
...
async function trackingAnalyze() {
  ...
  try {
    const r = await Engine.analyzeGame(serializeForBackend(snap));
    if (!r.ok) {
      const d = typeof r.detail === "string" ? r.detail : JSON.stringify(r.detail);
      throw new Error(d || `HTTP ${r.status}`);
    }
